...
def get_banner(year, term, campus="%", faculty="%", prof="%", crn="%"):
    ...
    response = requests.post(
        "https://selfservice.mun.ca/direct/hwswsltb.P_CourseResults",
        headers=headers,
        data=data,
    )
...
def parse_subject(subject_info, subject_name):
    ...
    for i in range(len(subject_lines))[0:]:
        if (
            len(subject_lines[i]) > 0
            and subject_lines[i][0] != " "
            and subject_lines[i][10:20] != "Laboratory"
        ):
            course_lines = [subject_lines[i]]
            j = i + 1
            while len(subject_lines[j]) > 0 and (
                subject_lines[j][0] == " " or subject_lines[j][10:20] == "Laboratory"
            ):
...
def parse_course(course_info, subject_name):
    ...
    for i in range(len(course_info)):
        if (
            len(course_info[i]) > 38
            and course_info[i][38] != " "
            and course_info[i][42:47].isdigit()
        ):
            offering_lines = [course_info[i]]
            j = i + 1
            while (
                j < len(course_info)
                and len(course_info[j]) > 38
                and course_info[j][38] == " "
            ):
                offering_lines.append(course_info[j])
                j += 1
            offerings[course_info[i][38:41]] = parse_offering(
                offering_lines, course_info[0][:5].strip(), subject_name
            )
    ...
    for line in offering_info:
        ...
        if is_note:
            if line[42:55] == "CROSS LISTED:":
                ...
                cross_listed = [
                    cross_listed_line[i : i + 13]
                    for i in range(0, len(cross_listed_line), 14)
                ]
...
def parse_time(time_string, time_dict):
    if time_string[0] == "M":
        time_dict["monday"].append(time_string[14:])
    if time_string[2] == "T":
        time_dict["tuesday"].append(time_string[14:])
    if time_string[4] == "W":
        time_dict["wednesday"].append(time_string[14:])
    if time_string[6] == "R":
        time_dict["thursday"].append(time_string[14:])
    if time_string[8] == "F":
        time_dict["friday"].append(time_string[14:])
    if time_string[10] == "S":
        time_dict["saturday"].append(time_string[14:])
    if time_string[12] == "U":
        time_dict["sunday"].append(time_string[14:])
